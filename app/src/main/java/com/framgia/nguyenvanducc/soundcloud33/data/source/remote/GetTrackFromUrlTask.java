...
public class GetTrackFromUrlTask extends BaseHttpTask<Track> {
    private static final String COLLECTION_ENTITY = "collection";
    private static final String TRACK_ENTITY = "track";
    ...
    public GetTrackFromUrlTask(OnLoadDataCompleteListener<Track> onLoadDataCompleteListener) {
    ...
    public List<Track> getJsonDataFromString(String string) throws JSONException {
        ...
        for (int i = 0; i < trackJsonArray.length(); i++) {
            JSONObject trackJson = trackJsonArray.getJSONObject(i).getJSONObject(TRACK_ENTITY);
